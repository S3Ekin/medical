$(function() {
	var myChart = echarts.init($('#pie_div_chart').get(0));
	...
	$.ajax({
		...
		success : function(area) {
			$('#pie_sel_area').combotree('loadData', area);
			// 加载窗口树
			$.ajax({
...
//	$('#pie_div_radius1').hide();
	$('#pie_sel_charttype').next('span').hide();
...
//				var keshi = $("#pie_sel_area").combotree('tree').tree('getSelected').id;
			...
			if(rate != "day"){
				...
				var keshi = "";
//					$("#pie_sel_area").combotree('tree').tree('getSelected').id;
