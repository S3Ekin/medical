...
//			// 加载窗口树
			...
			success : function(data){
				...
				if(nodes.length == 1){//如果只有一个Kpi 则没有公共dim标签页
					...
					for(var j=0; j<kpi_dims.length; j++){
						...
						var dim_type = dim[2];
						//alert(dim_id+" "+dim_name+" "+dim_type+" "+kpi_id);
						if(dim_type == "1"){//树结构 部门和地区 获取对应的下拉框数据
							$.ajax({
								dim_id : dim_id,
								dim_name : dim_name,
								url : "ajax/GetDimComboTree"+dim_id+".json",
	...
	$('#line_dg_kpi').datagrid({
		fitColumns : true,//表格自适应
		striped : true,//条文化
		singleSelect : false,//可以多行
		method : 'get',
    ...
    $('#line_button_makechart').click(function(){
        ...
        //获取选中的选项卡
        var _selectTab=$("#line_tabs").tabs("getSelected").panel("options").tab;
        lineoption.title.text=_selectTab.text();
