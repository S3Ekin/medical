	...
	$('#button_makechart').click(function(){
		...
		if(selValue == "1"){
			...
			if(rate != "day"){
				...
				var keshi = 1;//$("#funnel_sel_area").combotree('tree').tree('getSelected').id;
					...
					$.ajax({
						...
						success : function(data) {
							...
							if(title != ""){
								...
								funneloption.title.text = "财务科";//$("#funnel_sel_area").combotree('tree').tree('getSelected').text;
							}
		...
		if($('#funnel_sel_charttype').combobox('getValue') == "line"){
			$.ajax({
				url : "../ajax/GetLegendServlet?var0="+variable[0]+"&var1="+variable[1]+"&var2="+variable[2],
				dataType : "text",
				success : function(legend) {
					//向后台数据库传值并接受折现图信息
					$.ajax({
						url : "../ajax/PieChartServlet?var0="+variable[0]+"&var1="+variable[1]+"&var2="+variable[2]+"&type=chartline",
						dataType : "text",
						success : function(data) {
							//$('#funnel_text_title').textbox('setValue','');
							var title = $('#funnel_text_title').textbox('getValue');
							if(title != ""){
								...
								lineoption.title.text = "财务科";//$("#funnel_sel_area").combotree('tree').tree('getSelected').text;
							}
							//alert($('input:radio[name="isrose"]:checked').val());
							var roseType = $('input:radio[name="isrose"]:checked').val();
							var bool=false;
